...
class RevenueChartProcessor {
  ...
  static List<FlSpot> processRevenueData(List<Statistic> statistics, int year) {
    ...
    for (var statistic in statistics) {
      ...
      if (statistic.order.createdAt.toDate().year == year) {
        // Extract the month (0-based index)
        int month = statistic.order.createdAt.toDate().month - 1;

        // Add the total price of the order to the corresponding month
        monthlyRevenues[month] += (statistic.order.totalPrice) * 0.1;
      }
    }
